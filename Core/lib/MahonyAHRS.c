...
void MahonyAHRS_init(MahonyAHRS *obj, float _twoKp, float _twoKi, float _sampleFreq)
{
    obj->twoKp = _twoKp;
    obj->twoKi = _twoKi;
...
void MahonyAHRS_update(MahonyAHRS *obj, float gx, float gy, float gz, float ax, float ay, float az)
{
    float recipNorm;
    ...
    if (!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f)))
    {
        ...
        if (obj->twoKi > 0.0f)
        {
            obj->integralFBx += obj->twoKi * halfex * (1.0f / obj->sampleFreq); // integral error scaled by Ki
