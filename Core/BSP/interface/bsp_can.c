...
void BSP_CAN_Send(uint8_t can_id, uint16_t identifier, uint8_t *data, uint32_t len)
{
    ...
    txconf.DLC = len;
    HAL_CAN_AddTxMessage(can_devices[can_id].device, &txconf, data, can_devices[can_id].tx_mailbox);
}
...
void update_filter(uint8_t can_id, uint32_t filter_index)
{
    ...
    {
        if (filter_data[group_index * 4 + i] != ID_NOTSET)
            id_all_notset = 0;
    }
    if (id_all_notset)
