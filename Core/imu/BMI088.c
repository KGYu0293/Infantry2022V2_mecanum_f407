...
void spi_read_multi(SPI_HandleTypeDef *spi_port, uint8_t reg, uint8_t *buf, uint8_t len);
...
{
    //硬件接口参数
    obj->ACCEL_NS_BASE = GPIOA;
    obj->ACCEL_NS_PIN = GPIO_PIN_4;
    obj->GYRO_NS_BASE = GPIOB;
    obj->GYRO_NS_PIN = GPIO_PIN_0;
    obj->SPI_PORT = &hspi1;
    obj->HEAT_PWM_BASE = &htim10;
    obj->HAET_PWM_CHANNEL = TIM_CHANNEL_1;
    
    //设定温度为55度
    obj->temp_target = 55.0f;
    //初始化标志位
...
void BMI088_heat_control(BMI088_imu *obj)
{
    ...
    __HAL_TIM_SetCompare(obj->HEAT_PWM_BASE, obj->HAET_PWM_CHANNEL, (uint16_t)(obj->heat_pid.output));
}
