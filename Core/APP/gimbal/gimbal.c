...
Gimbal *Gimbal_Create() {
    ...
    PID_SetConfig_Pos(&yaw_controller_config.speed_pid_config, 240, 3, 6, 2000, 25000);
    //PID_SetConfig_Pos(&yaw_controller_config.position_pid_config, 17.6, 0.1, 6, 141, 5000);
    //PID_SetConfig_Pos(&yaw_controller_config.speed_pid_config, 140, 3, 8, 2000, 25000);
    ...
    PID_SetConfig_Pos(&pitch_controller_config.position_pid_config, 37, 0.068, 18.41, 109.9, 5000);
...
void Gimbal_Update(Gimbal *gimbal) {
    ...
    // 重要外设掉线检测
    if ((gimbal->imu->monitor->count < 1) || !(gimbal->imu->bias_init_success) || gimbal->yaw->monitor->count < 1) {
    ...
    // 模块控制
    if (gimbal->cmd_data->mode == gimbal_stop) {
        ...
        switch (gimbal->cmd_data->mode) {
            ...
            case gimbal_middle:
                ...
                gimbal->yaw->motor_controller->ref_position = gimbal->cmd_data->yaw;
                // pitch轴
                gimbal->pitch->motor_controller->ref_position = gimbal->imu->data.euler_deg[1] + (gimbal->pitch->fdbPosition - PITCH_MOTOR_ENCORDER_BIAS) * 360.0 / 8192;
                // // 设定编码器控制
